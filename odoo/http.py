...
class Request:
    ...
    def validate_csrf(self, csrf):
        ...
        """
        if not csrf:
            _logger.warning("CSRF validation failed: no token provided")
            return False
        ...
        if max_ts:
            try:
                if int(max_ts) < int(time.time()):
                    _logger.warning("CSRF validation failed: token expired (max_ts: %s, current: %s)", max_ts, int(time.time()))
                    return False
            except ValueError:
                _logger.warning("CSRF validation failed: invalid max_ts %s", max_ts)
                return False

        hm_expected = hmac.new(secret.encode('ascii'), msg, hashlib.sha1).hexdigest()
        result = consteq(hm, hm_expected)
        if not result:
            _logger.warning("CSRF validation failed: token mismatch. session_id: %s, expected_hm: %s, received_hm: %s", self.session.sid[:8], hm_expected, hm)
        return result
