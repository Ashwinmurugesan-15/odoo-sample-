...
class DiscussChannelMember(models.Model):
    ...
    def _search_is_pinned(self, operator, operand):
        if operator not in ("=", "!=") or not isinstance(operand, bool):
             return NotImplemented
        want_pinned = (operator == "=" and operand) or (operator == "!=" and not operand)
        self.env.cr.execute("""SELECT id FROM discuss_channel_member WHERE unpin_dt IS NULL OR (last_interest_dt IS NOT NULL AND last_interest_dt >= unpin_dt)""")
        pinned_ids = [r[0] for r in self.env.cr.fetchall()]
        return [("id", "in", pinned_ids)] if want_pinned else [("id", "not in", pinned_ids)]
